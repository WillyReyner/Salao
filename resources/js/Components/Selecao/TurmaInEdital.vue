<template>
    ...
    <div class="tw-flex tw-flex-1 tw-flex-col">
        <div
            class="tw-flex tw-flex-1 tw-flex-col tw-justify-start tw-bg-white tw-shadow-none sm:tw-rounded-lg tw-px-8 tw-py-4 tw-overflow-hidden tw-overflow-y-scroll">
                ...
                <div class="tw-flex tw-flex-1 tw-flex-col tw-justify-end tw-items-center tw-gap-6 md:tw-flex-row tw-pt-5">
                    ...
                    <div
                        ...
                        <input id="busca" type="text" placeholder="Pesquise aqui" required autofocus autocomplete="busca"
                            class="tw-appearance-none tw-border-none tw-w-full tw-text-gray-700" v-model="busca" />
                    </div>
                </div>
            </div>
            <!-- Tabela de Exibição -->
            <div class="tw-antialiased tw-font-body">
                <div class="tw-py-4 tw-overflow-x-auto">
                    <div class="tw-flex tw-flex-1 tw-flex-col tw-overflow-hidden">
                        <q-table wrap-cells flat :rows="filteredTurmas" :columns="columns" :filter="busca"
                                ...
                                <q-tr :props="props" no-hover
                                    ...
                                    <q-td v-for="col in props.cols" :key="col.name" :props="props">
                                        <div v-if="col.name === 'actions'">
                                            <span
                                                @click="props.row.edital_id !== edital_id ? reaproveitar(props.row) : null"
                                                :class="{ 'clickable': props.row.edital_id !== edital_id, 'tw-text-green-500 tw-cursor-auto': props.row.edital_id === edital_id }"
                                                class="tw-font-title tw-text-gray-600 tw-text-lg tw-cursor-pointer hover:tw-opacity-90">
                                                {{ props.row.edital_id !== edital_id ? 'REAPROVEITAR' : 'VINCULADO' }}
                                            </span>
                                        </div>
...
import {evaArrowBackOutline, evaCloseCircleOutline} from '@quasar/extras/eva-icons';
...
function reaproveitar(turma) {
    ...
    formR.post(route('painel.turmas.reuse'), {
        preserveScroll: true,
        forceFormData: true,
        onSuccess: (res) => {
            formR.reset();
