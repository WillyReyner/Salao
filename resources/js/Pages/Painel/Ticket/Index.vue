		...
		<div class="tw-flex tw-flex-1 tw-flex-col">
			<div
				...
				<div
					...
					<h4 class="tw-text-gray-900 tw-font-title tw-font-semibold">Tickets</h4>
					<Link :href="route('painel.ticket.create')">
												...
												<PrimaryButton class="tw-text-black tw-text-lg">...
													<q-menu auto-close>
														<div class="tw-flex tw-flex-col tw-gap-2">
															<Link :href="route('painel.ticket.show', props.row.id)"
															...
															<!-- <Link :href="route('painel.ticket.log', props.row.id)" title="Info"
																 class="tw-flex hover:tw-bg-gray-100 flex-row tw-p-2 z-10 tw-justify-right tw-items-center">
																 <Icon icon="arcticons:tenbitclockwidget" class="tw-h-6 tw-w-6" />
																 &nbsp;Histórico de Logs
															</Link> -->
															<Link :href="route('painel.ticket.editar', props.row.id)"
															      class="tw-flex hover:tw-bg-gray-100 flex-row tw-p-2 z-10 tw-justify-right tw-items-center"
															      title="Editar">
...
const columns = [
	...
	{name: 'nome', align: 'left', label: 'TICKET', field: 'nome', sortable: true},
	{
		name: 'created_at',
		field: 'created_at',
		required: true,
...
const filteredTipos = ref(props.ticket)
...
function applyFilters() {
	const filtradoCriacao = filtroCriacao.value != '' ? props.ticket.filter(tipo => tipo.created_at.split('T')[0] == filtroCriacao.value) : props.ticket
...
function removeItem(item) {
	...
	form.post(route('painel.ticket.destroy'), {
		preserveScroll: true,
		onSuccess: (res) => {
			Notify.create({
