...
const responsaveisList = props.responsaveis.map(resp => ({ label: resp.name, value: resp.id }))
...
function validateForm() {
	const fieldLabels = {
		...
		contato: 'Contato',
		user_id: 'Responsável pela Escola',
...
function store() {
	...
	form.post(route('painel.escolas.store'), {
		preserveScroll: true,
		onSuccess: (res) => {
			form.reset()
			showModal.value = false
			Notify.create({
				message: "Escola registrado com sucesso.",
				color: "secondary"
			})
		},
		onError: (err) => {
			Notify.create({
				message: "Não foi possível registrar a Escola.",
				color: "negative"
			})
		}
	})
...
<template>
    ...
    <AuthenticatedLayout>
        <div class="tw-flex tw-flex-1 tw-flex-col">
            <div
                class="tw-flex tw-flex-1 tw-flex-col tw-justify-start tw-bg-white tw-shadow sm:tw-rounded-lg tw-px-8 tw-py-4 tw-overflow-y-scroll">
                <div
                    ...
                    <h4 class="tw-font-title tw-font-semibold">Adicionar Unidade</h4>
                </div>
                ...
                <div class="tw-flex md:tw-flex-row tw-flex-col md: tw-gap-2 tw-m-2">
                    <div class="tw-flex tw-flex-1 tw-flex-col">
                        ...
                        <q-select outlined map-options emit-value :options="municipiosList" v-model="form.municipio_id"
                            id="municipio_id"
                            class="tw-mt-1 tw-text-gray-700 tw-h-[52px] tw-block tw-w-full tw-border-gray-200 focus:tw-border-gray-200 focus:tw-ring-0 tw-rounded-lg tw-shadow-sm" />
                    ...
                    <div class="tw-flex tw-flex-1 tw-flex-col">
                        ...
                        <q-input outlined id="contato" type="text" mask="(##) #####-####" required autofocus
                    ...
                    <div class="tw-flex tw-flex-1 tw-flex-col">
                        ...
                        <q-input outlined id="ficha" type="text" required autofocus autocomplete="ficha"
                            class="tw-mt-1 tw-h-[52px] tw-block tw-w-full tw-border-gray-200 focus:tw-border-gray-500 focus:tw-ring-0 tw-rounded-lg tw-shadow-sm"
                            v-model="form.ficha" />
                        <InputError class="tw-mt-2" :message="form.errors.ficha" />
                    </div>
                </div>
                <div class="tw-flex md:tw-flex-row tw-flex-col md: tw-gap-2 tw-m-2 tw-pb-4">
                    <div class="tw-flex tw-flex-1 tw-flex-col">
                        <InputLabel for="user_id" value="Responsáveis" required />
                        <q-select name="user_id" outlined multiple :options="responsaveisList" stack-label map-options
                            emit-value v-model="form.user_id"
                            class="tw-mt-1 tw-h-[52px] tw-block tw-w-full tw-border-gray-200 focus:tw-border-gray-500 focus:tw-ring-0 tw-rounded-lg tw-shadow-sm">
