        ...
        <div class="tw-flex tw-flex-1 tw-flex-col">
            <div
                ...
                <div
                    ...
                    <h4 class="tw-text-gray-900 tw-font-title tw-font-semibold">Tags Cadastradas</h4>
                    <div
                        class="tw-flex  md:tw-flex-row tw-flex-col tw-gap-4 tw-py-4 tw-items-center tw-justify-between items">
                        <h5 v-if="hasPermission('Importar Tags')"
                        ...
                        <Link :href="route('painel.tags.create')">
                        <PrimaryButton v-if="hasPermission('Criar Tags')" class="tw-text-white tw-bg-green-500 tw-items-center">
                ...
                <div class="tw-flex md:tw-flex-row tw-flex-col md:tw-gap-10 tw-gap-2 tw-mb-8 tw-items-center">
                    ...
                    <div
                        ...
                        <div class="tw-flex tw-flex-1 tw-flex-row tw-gap-5 tw-justify-end tw-items-center">
                            ...
                            <h5 v-if="hasPermission('Exportar Tags')" @click="exportToCSV"
                                class="tw-text-gray-700 tw-cursor-pointer tw-flex tw-flex-row tw-text-lg tw-font-body tw-justify-center tw-gap-2">
                ...
                <div class="tw-antialiased tw-font-body">
                    <div class="tw-py-4 tw-overflow-x-auto">
                        <div class="tw-flex tw-flex-1 tw-flex-col tw-overflow-hidden">
                            <q-table flat v-model:rows="filteredTags" :columns="columns" :visible-columns="visibleColumns"
                                                ...
                                                <PrimaryButton class="tw-text-black tw-text-lg">...
                                                    <q-menu auto-close>
                                                        <div class="tw-flex tw-flex-col tw-gap-2">
                                                            ...
                                                            <Link v-if="hasPermission('Editar Tags')" :href="route('painel.tags.editar', props.row.id)"
                                                                title="Editar"
                                                                class="tw-flex hover:tw-bg-gray-100 flex-row tw-p-2 z-10 tw-justify-right tw-items-center">
                                                            <Icon icon="arcticons:simpletexteditor" class="tw-h-6 tw-w-6" />
                                                            &nbsp;Editar
                                                            </Link>
                                                            <span v-if="hasPermission('Excluir Tags')" @click="confirmRemoving(props.row)" title="Deletar"
...
function applyFilters() {
    ...
    filteredTags.value = filtradoNome
    filtersModal.value = false
}
...
function removeItem(item) {
    ...
    form.delete(route('painel.tags.destroy'), {
        preserveScroll: true,
        onSuccess: (res) => {
            Notify.create({
...
const exportToCSV = () => {
    ...
    if (rowsSelected.value.length === 0) {
        ...
        const data = filteredTags.value.map((role) => [
            role.id,
            `"${role.nome}"`,
            role.situacao
        ]);
        ...
        link.setAttribute('download', 'TagsExport.csv');
