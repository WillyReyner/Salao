			...
			<div
				...
				<div class="tw-flex md:tw-flex-row tw-flex-col md:tw-gap-10 tw-gap-2 tw-m-2">
					<div class="tw-flex tw-flex-1 tw-flex-col">
						...
						<q-input id="orientacao" v-model="form.orientacao" autocomplete="orientacao" autofocus
						         class="tw-mt-1 tw-block tw-w-full tw-border-gray-200 focus:tw-border-gray-500 focus:tw-ring-0 tw-rounded-lg tw-shadow-sm"
						         hint="Ao colocar a orientação, colocar '&' no final para que seja feito a quebra de linha. Ex: Teste& Teste& Teste"
						         outlined
				...
				<div class="tw-flex tw-flex-auto md:tw-flex-row tw-flex-col md:tw-gap-10 tw-gap-2 tw-mb-4">
					<div class="tw-flex tw-flex-1 tw-flex-col">
						...
						<q-select v-model="form.tipo_arquivo" :options="optionsTipo" behavior="menu"
						          class="tw-mt-1 tw-h-[52px] tw-block tw-w-full tw-border-gray-200 focus:tw-border-gray-500 focus:tw-ring-0 tw-rounded-lg tw-shadow-sm"
						          input-debounce="0"
						          outlined use-input
						          @filter="filterFn">
...
function store() {
	...
	form.post(route('painel.documento.store'), {
		preserveScroll: true,
		onSuccess: (res) => {
			form.reset()
			Notify.create({
