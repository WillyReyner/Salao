...
function store(){
    ...
    form.post(route('painel.areas.store'), {
        preserveScroll: true,
        onSuccess: (res) => {
            form.reset()
            Notify.create({
                message: "Area registrada com sucesso.",
                color: "secondary"
            })
        },
        onError: (err) => {
            console.log(err)
            Notify.create({
                message: "Não foi possível registrar a Area.",
                color: "negative"
            })
        }
    })
...
<template>
    <Head title="Áreas" />
    <AuthenticatedLayout>
	    <q-dialog v-model="openSubAreas" class="">
		    <q-card class="tw-flex tw-flex-1 tw-p-6 tw-gap-5 tw-rounded-2xl">
			    <q-card-section class="tw-flex tw-flex-1 tw-flex-col tw-gap-5 tw-border tw-border-red-500">
				    <div>
			    ...
			    <div class="tw-flex md:tw-flex-col md:tw-gap-4 tw-gap-2 tw-m-2 tw-pb-4">
				    ...
				    <div class="tw-col-span-12 md:tw-col-span-6 lg:tw-col-span-3">
					    ...
					    <div class="tw-flex tw-gap-2 tw-justify-between">
						    <InputLabel for="subareas" required value="Subáreas"/>
						    <span @click="openSubAreas = true" class="tw-cursor-pointer tw-border tw-border-green-800 tw-px-1 tw-rounded-2xl ">+
						   </span>
					    </div >
					    <q-select v-model="form.subAreas" :options="optionsSubAreas" behavior="menu"
