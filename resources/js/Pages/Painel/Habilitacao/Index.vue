        ...
        <div class="tw-flex tw-flex-1 tw-flex-col">
            <div
                ...
                <div
                    ...
                    <h4 class="tw-text-gray-900 tw-font-title tw-font-semibold">Habilitações Cadastradas</h4>
                    <div
                        class="tw-flex  md:tw-flex-row tw-flex-col tw-gap-4 tw-py-4 tw-items-center tw-justify-between items">
                        <h5 v-if="hasPermission('Importar Habilitacao')"
                        ...
                        <Link :href="route('painel.habilitacao.create')">
                        <PrimaryButton v-if="hasPermission('Criar Habilitacao')" class="tw-text-white tw-bg-green-500 tw-items-center">
                ...
                <div class="tw-flex md:tw-flex-row tw-flex-col md:tw-gap-10 tw-gap-2 tw-mb-8 tw-items-center">
                    ...
                    <div
                        ...
                        <div class="tw-flex tw-flex-1 tw-flex-row tw-gap-5 tw-justify-end tw-items-center">
                            ...
                            <h5 v-if="hasPermission('Exportar Habilitacao')" @click="exportToCSV"
                                class="tw-text-gray-700 tw-cursor-pointer tw-flex tw-flex-row tw-text-lg tw-font-body tw-justify-center tw-gap-2">
                ...
                <div class="tw-antialiased tw-font-body">
                    <div class="tw-py-4 tw-overflow-x-auto">
                        <div class="tw-flex tw-flex-1 tw-flex-col tw-overflow-hidden">
                            <q-table flat v-model:rows="filteredHabilitacao" :columns="columns" :visible-columns="visibleColumns"
                                                ...
                                                <PrimaryButton class="tw-text-black tw-text-lg">...
                                                    <q-menu auto-close>
                                                        <div class="tw-flex tw-flex-col tw-gap-2">
                                                            <Link :href="route('painel.habilitacao.show', props.row.id)"
                                                                title="Info"
                                                            ...
                                                            <!-- <Link :href="route('painel.tags.show', props.row.id)"
                                                                title="Info"
                                                                class="tw-flex hover:tw-bg-gray-100 flex-row tw-p-2 z-10 tw-justify-right tw-items-center">
                                                            ...
                                                            <Link v-if="hasPermission('Editar Habilitacao')" :href="route('painel.habilitacao.editar', props.row.id)"
                                                                title="Editar"
                                                                class="tw-flex hover:tw-bg-gray-100 flex-row tw-p-2 z-10 tw-justify-right tw-items-center">
...
const columns = [
    ...
    { name: 'nome', align: 'left', label: 'HABILITAÇÃO', field: 'nome', sortable: true },
...
const filteredHabilitacao = ref(props.habilitacao)
...
function applyFilters() {
    ...
    filteredHabilitacao.value = filtradoNome
    filtersModal.value = false
}
...
function removeItem(item) {
    ...
    form.delete(route('painel.habilitacao.destroy'), {
        preserveScroll: true,
        onSuccess: (res) => {
            Notify.create({
                message: "Tipo(s) de Habilitação(ões) removido(s) com sucesso.",
                color: "secondary"
            })
        ...
        onError: (err) => {
            Notify.create({
                message: "Não foi possível remover esse(s) Tipo(s) de Habilitação(ões).",
                color: "negative"
            })
        }
    })
...
const exportToCSV = () => {
    ...
    if (rowsSelected.value.length === 0) {
        ...
        const data = filteredHabilitacao.value.map((role) => [
            role.id,
            `"${role.nome}"`,
            role.situacao
        ]);
        ...
        link.setAttribute('download', 'HabilitaçõesExport.csv');
