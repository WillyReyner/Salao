...
const logLabels = {
    nome: "Nome",
    id_curso_siga: "Id do Curso do Siga",
    escola_curso_siga: "Escola do Siga",
    situacao: "Situação",
    link_video_apresentacao: "Link do Video de Apresentação",
    imagem: "Imagem",
    destaque: "Destaque",
    objetivos: "Objetivos",
    conteudo_programatico: "Conteudo Programatico",
    campo_atuacao: "Campo de Atuação",
    tag: "Tags",
    descricao: "Descricao",
    categoria: "Categoria",
    horas_curso: "Qnt de horas do curso",
    created_by: "Criado por quem?",
    updated_by: "Atualizado por quem?",
    created_at: "Criado quando?",
    updated_at: "Atualizado quando?",
    deleted_at: "Deletado quando?",
}
        ...
        <div class="tw-flex tw-flex-1 tw-flex-col tw-overflow-y-scroll">
            <div class="tw-flex tw-flex-1 tw-flex-col tw-justify-start tw-bg-white tw-shadow sm:tw-rounded-lg tw-px-8 tw-py-4">
                <div class="tw-flex md:tw-flex-row tw-flex-col tw-gap-4 tw-py-4 tw-items-center tw-justify-between items tw-border-b-2 tw-border-gray-200">
                    <h4 class=" tw-font-title tw-font-semibold">Histórico Geral do {{ nome ? nome : 'Sistema' }}</h4>
                </div>
                        ...
                        <div class="tw-flex tw-flex-1 tw-flex-col tw-overflow-hidden">
                            <q-table
                                ...
                                <template v-slot:header="props">
                                    <q-tr :props="inscricao" class="tw-h-20">
                                            ...
                                            <div v-else-if="col.name == 'properties'">
                                                <div v-for="(value, key) in props.row.properties.attributes">
                                                    <div v-if="key == 'escola_id'">
                                                        {{ logLabels[key] || key }}: {{ inscricao.find(item => item.id == value).nome || value }}
                                                    </div>
