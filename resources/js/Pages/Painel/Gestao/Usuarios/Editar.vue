    ...
    <div class="tw-flex tw-flex-1 tw-flex-col">
      <div
        ...
        <div class="tw-flex md:tw-flex-row tw-flex-col md:tw-gap-8 tw-gap-2 tw-m-1 tw-pb-4">
              ...
              <q-file outlined :model-value="form.imagem" @update:model-value="updateFile" accept=".jpg,.png">
                <template v-slot:before>
                  <div class="q-file-image-preview" :style="getImagePreviewStyle">
                    <!-- Conditionally render content based on updateFile -->
          ...
          <div class="tw-flex tw-flex-1 tw-flex-col">
            <div class="tw-flex md:tw-flex-row tw-flex-col md:tw-gap-10 tw-gap-2 tw-m-2 tw-pb-14">
              ...
              <div class="tw-flex tw-flex-col tw-mr-4 tw-w-1/4">
                ...
                <q-select outlined emit-value map-options :options="opcoesRoles" id="role" v-model="form.role"
                  class="tw-mt-1 tw-text-gray-700 tw-h-[52px] tw-block tw-w-full tw-border-gray-200 focus:tw-border-gray-200 focus:tw-ring-0 tw-rounded-lg tw-shadow-sm" />
...
const opcoesRoles = props.roles.map(role => ({ label: role.name, value: role.id }))
const opcoesPermissions = props.permissions.map(permission => ({ label: permission.name, value: permission.id }))

const opcoesTag = ['Super Admin', 'Admin', 'Aluno', 'Usuário', 'Escola'];
const uploadUrl = ref("");
const destaque = ref("");
...
const imagemBd = ref(`/storage/+ ${props.usuarios}`)
