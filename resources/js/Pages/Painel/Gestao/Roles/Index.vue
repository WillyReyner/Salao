        ...
        <div class="tw-flex tw-flex-1 tw-flex-col">
            <div
                ...
                <div
                    ...
                    <h4 class="tw-text-gray-900 tw-font-title tw-font-semibold">Níveis de Acessos Cadastrados</h4>
                    <Link :href="route('painel.gestao.roles.create')">
                ...
                <div class="tw-antialiased tw-font-body">
                    <div class="tw-py-4 tw-overflow-x-auto">
                        <div class="tw-flex tw-flex-1 tw-flex-col tw-overflow-hidden">
                            <q-table flat wrap-cells v-model:rows="filteredRoles" :columns="columns"
                                                ...
                                                <PrimaryButton class="tw-text-black tw-text-lg">...
                                                    <q-menu auto-close>
                                                        <div class="tw-flex tw-flex-col tw-gap-2">
                                                            <!-- <Link :href="route('painel.gestao.roles.show', props.row.id)"
                                                                title="Info"
                                                            ...
                                                            <Link :href="route('painel.gestao.roles.editar', props.row.id)"
                                                                title="Editar"
                                                                class="tw-flex hover:tw-bg-gray-100 flex-row tw-p-2 z-10 tw-justify-right tw-items-center">
...
// import Permissoes from '@/Utils/CheckPermission'
...
function removeItem(item) {
    ...
    form.post(route('painel.gestao.roles.destroy', form.id), {
        preserveScroll: true,
        onSuccess: (res) => {
            Notify.create({
            ...
            window.location.href = '/gestao/roles';
            applyFilters()
        },
        onError: (err) => {
            Notify.create({
...
const exportToCSV = () => {
    ...
    if (rowsSelected.value.length === 0) {
        const columns = [
            'ID',
            'NOME DOS NÍVEIS DE ACESSO',
        ];
        const data = filteredRoles.value.map((role) => [
            role.id,
            `"${role.name}"`,
        ]);
        ...
        link.setAttribute('download', 'RolesExport.csv');
