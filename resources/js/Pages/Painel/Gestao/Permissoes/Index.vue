                    ...
                    <div class="tw-flex tw-flex-1 tw-flex-row tw-gap-4">
                        ...
                        <div class="tw-flex tw-flex-1 tw-flex-row tw-gap-6">
                            ...
                            <div class="tw-flex tw-flex-1 tw-flex-col">
                                ...
                                <select v-model="filtroMunicipio" id="escola"
        ...
        <div class="tw-flex tw-flex-1 tw-flex-col">
            <div
                ...
                <div
                    ...
                    <h4 class="tw-text-gray-900 tw-font-title tw-font-semibold">Permissões Cadastradas</h4>
                    <Link :href="route('painel.gestao.permissoes.create')">
                ...
                <div class="tw-flex md:tw-flex-row tw-flex-col md:tw-gap-10 tw-gap-2 tw-mb-8 tw-items-center">
                    ...
                    <div class="tw-flex tw-flex-1 tw-flex-row tw-items-center tw-gap-2 md:tw-flex-row tw-pt-5">
                        <q-chip size="lg" :icon="evaCloseCircleOutline" outline clickable v-if="chipMunicipio"
                            @click="removeFilterMunicipio" color="green" text-color="green">
                            Municipio
                        </q-chip>
                    ...
                    <div
                        ...
                        <div class="tw-flex tw-flex-1 tw-flex-row tw-gap-5 tw-justify-end tw-items-center">
                            <!--<h5 @click="filtersModal = true" class="tw-text-gray-700 tw-cursor-pointer tw-flex tw-flex-row tw-text-lg tw-font-body tw-justify-center tw-gap-2">
                ...
                <div class="tw-antialiased tw-font-body">
                    <div class="tw-py-4 tw-overflow-x-auto">
                        <div class="tw-flex tw-flex-1 tw-flex-col tw-overflow-hidden">
                            <q-table flat wrap-cells v-model:rows="filteredPermissoes" :columns="columns"
                                :visible-columns="visibleColumns" row-key="id" :filter="busca"
                                                ...
                                                <PrimaryButton class="tw-text-black tw-text-lg">...
                                                    <q-menu auto-close>
                                                        <div class="tw-flex tw-flex-col tw-gap-2">
                                                            <Link
                                                                :href="route('painel.gestao.permissoes.show', props.row.id)"
                                                                title="Info"
                                                            ...
                                                            <Link
                                                                :href="route('painel.gestao.permissoes.editar', props.row.id)"
                                                                title="Editar"
                                                                class="tw-flex hover:tw-bg-gray-100 flex-row tw-p-2 z-10 tw-justify-right tw-items-center">
...
const columns = [
    ...
    { name: 'created_at', field: 'created_at', required: true, label: 'DATA DE CRIAÇÃO', format: val => `${new Date(val).toLocaleString('pt-BR', dateFormatOptions)}`, align: 'left', sortable: true },
    { name: 'actions', label: 'AÇÕES', align: 'center', field: 'id' },
]

const visibleColumns = [
    'nome', 'created_at', 'actions'
]

const filtroCriacao = ref('')
const filtroMunicipio = ref('')
const filteredPermissoes = ref(props.permissions)
...
function removeItem(item) {
    ...
    form.post(route('painel.gestao.permissoes.destroy', form.id), {
        preserveScroll: true,
        onSuccess: (res) => {
            Notify.create({
                message: "Permissão(ões) removido(s) com sucesso.",
                color: "secondary"
            })
            window.location.href = '/gestao/permissoes';
            applyFilters()
        },
        onError: (err) => {
            Notify.create({
                message: "Não foi possível remover essas(s) Permissão(ões).",
                color: "negative"
            })
        }
    })
...
const exportToCSV = () => {
    ...
    if (rowsSelected.value.length === 0) {
        ...
        const data = filteredPermissoes.value.map((role) => [
            role.id,
            `"${role.name}"`,
        ]);
        ...
        link.setAttribute('download', 'PermissionsExport.csv');
